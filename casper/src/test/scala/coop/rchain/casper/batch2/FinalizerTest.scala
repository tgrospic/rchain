  ...
  it should "not advance finalization if no new LFB found, advance otherwise, invoke all effects" in withStorage {
    implicit blockStore => implicit blockDagStorage =>
      ...
      for {
        ...
        lfb <- Finalizer.run[Task, BlockHash, ByteString](
                dag,
                faultToleranceThreshold = -1,
                lastFinalizedHeight = 0L,
                lms
              )(
                m => (lfbStore = m).pure[Task],
                m => finalisedStore.update(v => v + m),
                dag.isFinalized,
                m => dag.lookupUnsafe(m).map(_.weightMap),
                m => dag.lookupUnsafe(m).map(_.blockNum),
                m => dag.toCasperJustificationUnsafe(m),
                m => dag.getCasperJustificationsUnsafe(m)
              )
        ...
        lfb <- Finalizer.run[Task, BlockHash, ByteString](
                dag,
                faultToleranceThreshold = -1,
                lastFinalizedHeight = finalizedHeight,
                lms
              )(
