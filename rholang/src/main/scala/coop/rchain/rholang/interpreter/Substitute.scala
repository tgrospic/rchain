...
object Substitute {
  ...
  def substituteAndCharge[A: Chargeable, M[_]: Substitute[?[_], A]: Sync: CostStateRef](
      term: A,
      depth: Int,
      env: Env[Par]
  ): M[A] =
  ...
  def substituteNoSortAndCharge[A: Chargeable, M[_]: Substitute[?[_], A]: Sync: CostStateRef](
      term: A,
      depth: Int,
      env: Env[Par]
  ): M[A] =
