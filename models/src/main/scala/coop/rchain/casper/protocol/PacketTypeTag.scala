...
object PacketTypeTag extends Enum[PacketTypeTag] {
  ...
  object ValueOf {
    ...
    // Last finalized state messages
    implicit val valueOfStoreItemsMessageRequest: ValueOf[StoreItemsMessageRequest.type] = summon(
      StoreItemsMessageRequest
    )
    implicit val valueOfStoreItemsMessage: ValueOf[StoreItemsMessage.type] = summon(
      StoreItemsMessage
    )
  }
